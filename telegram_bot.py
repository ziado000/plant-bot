...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /stats command - send statistics file"""
    user_id = update.effective_user.id
    
    # Check if statistics file exists
    stats_file = Path('bot_statistics.csv')
    ...
    try:
        ...
        await update.message.reply_document(
            ...
            filename='bot_statistics.csv',
            caption=(
                f"📊 <b>إحصائيات البوت</b>\n"
                f"📁 الحجم: {file_size / 1024:.1f} KB\n\n"
                f"يمكنك فتح الملف في Excel أو Google Sheets\n\n"
                f"<i>Bot Statistics - Open in Excel or Google Sheets</i>"
            ),
            parse_mode='HTML'
        )
        print(f"📊 Stats file sent to user {user_id}")
        
    except Exception as e:
        print(f"❌ Error sending stats: {e}")
        await update.message.reply_text(
            "❌ حدث خطأ في إرسال الإحصائيات.\n"
            "Error sending statistics file."
        )
...
def main():
    ...
    async def post_init(app: Application):
        ...
        await app.bot.set_my_commands([
            ...
            BotCommand("stats", "📊 تحميل الإحصائيات"),
